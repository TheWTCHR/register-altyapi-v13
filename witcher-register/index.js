...
GuildMember.prototype.setRole = function (roller = [], reason) {
  let roles = this.roles.cache.clone().filter(e => e.managed).map(e => e.id).concat(roller);
  return this.roles.set(roles, reason).catch(err => {});
}